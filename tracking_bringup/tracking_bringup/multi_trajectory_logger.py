...
class RobotLog:
    ...
    def _write_row(self, t_sec: float, source: str):
        nan = float("nan")
        v = self.last_visual or {"stamp": nan, "x": nan, "y": nan, "yaw": nan}
        g = self.last_gt or {"stamp": nan, "x": nan, "y": nan, "yaw": nan}
        self.writer.writerow([
            self.sample_idx,
            f"{t_sec:.6f}",
            source,
...
class MultiTrajectoryLogger(Node):
    def __init__(self):
        ...
        for rid in self.robot_ids:
            ...
            self.create_subscription(PoseStamped, v_topic, self._make_visual_cb(rid), 10)
            self.create_subscription(PoseStamped, g_topic, self._make_gt_cb(rid), 10)
